...
(function() {
  ...
  function layoutLevels() {
    ...
    if (mapOverlay) {
      ...
      // Keep clear of the left/right arrow buttons too.
      const arrL = rectOf('#mapArrowLeft'), arrR = rectOf('#mapArrowRight');
      if (arrL) PAD_L = clampN(arrL.right + 14, 60, mapW * 0.22);
      if (arrR) PAD_R = clampN((mapW - arrR.left) + 14, 60, mapW * 0.22);
    }
    ...
    const orbitR = Math.max(70, Math.min(safeW, safeH) / 2 * 0.86 * factor);

    // Node size scales with the ring radius so 10 nodes always fit without
    // overlapping, on any screen size.
    const circumPerNode = (2 * Math.PI * orbitR) / LEVELS_PER_WORLD;
    const rTarget = clampN(circumPerNode * 0.34, 10, 34);
    ...
    for (let i = 0; i < LEVELS_PER_WORLD; i++) {
      ...
      const a = -Math.PI / 2 + (i / LEVELS_PER_WORLD) * Math.PI * 2;
      level.x = Math.round(cx + Math.cos(a) * orbitR);
      level.y = Math.round(cy + Math.sin(a) * orbitR);
    }
    worldRender[w].cx = cx; worldRender[w].cy = cy; worldRender[w].spread = orbitR;
  ...
  function drawActiveWorldZone() {
    ...
    ctx.setLineDash([5, 7]);
    ctx.beginPath(); ctx.arc(pos.cx, pos.cy, R, 0, Math.PI * 2); ctx.stroke();
    ctx.setLineDash([]);
    ctx.restore();
  ...
  function updateDOM() {
    ...
    if (clearedMaxEl) clearedMaxEl.textContent = _rbDoneForMax ? '110' : '100';
    const starsMaxEl = document.getElementById('mapStarsMax');
    if (starsMaxEl) starsMaxEl.textContent = _rbDoneForMax ? '330' : '300';
    const shardsMaxEl = document.getElementById('mapShardsMax');
    if (shardsMaxEl) shardsMaxEl.textContent = _rbDoneForMax ? '330' : '300';
