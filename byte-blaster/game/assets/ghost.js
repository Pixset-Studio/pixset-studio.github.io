...
(function () {
  ...
  function draw(ctx) {
    ...
    const at = play.i / STEP;
    const i = Math.floor(at);
    if (i >= play.pts.length) return;
    const a = play.pts[i], b = play.pts[Math.min(i + 1, play.pts.length - 1)];
    const t = at - i;
  ...
  async function finish(score, ok) {
    ...
    // с половины уровня. Но рекорд перезаписываем только лучшим результатом.
    const store = readStore();
    const old = store[key];
    if (!old || (old.score | 0) <= (score | 0)) {
      store[key] = { data, score: score | 0, at: Date.now() };
      writeStore(store);
    }
