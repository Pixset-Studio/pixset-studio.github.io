...
(function () {
  ...
  function apply(id, persist) {
    ...
    // Тему выбирают в двух местах: кнопкой в шапке и списком в настройках
    // аккаунта. Тот, кого не трогали, о смене не знал — в настройках так и
    // висела отметка «выбрана» на прошлой теме. Теперь о смене объявляется
    // всем, как это уже сделано со сменой языка.
    try {
      document.dispatchEvent(new CustomEvent('pixset:theme', { detail: id }));
    } catch (e) { /* очень старый браузер — обойдётся без обновления списка */ }
    return true;
  }
